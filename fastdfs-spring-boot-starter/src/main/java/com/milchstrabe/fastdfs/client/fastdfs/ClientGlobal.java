...
import com.milchstrabe.fastdfs.client.common.IniFileReader;
import com.milchstrabe.fastdfs.client.common.MyException;
import com.milchstrabe.fastdfs.client.config.FastdfsHttpProperties;
import com.milchstrabe.fastdfs.client.config.FastdfsPoolProperties;
import com.milchstrabe.fastdfs.client.config.FastdfsProperties;
...
public class ClientGlobal {
   ...
   *
   */
  public static void init(FastdfsProperties fastdfsProperties,
                          FastdfsPoolProperties fastdfsPoolProperties,
                          FastdfsHttpProperties fastdfsHttpProperties) throws MyException {
      String[] szTrackerServers;
      String[] parts;
      g_connect_timeout = fastdfsProperties.getConnectTimeout();
      if (g_connect_timeout < 0) {
        g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
      }
      g_connect_timeout *= 1000; //millisecond

      g_network_timeout = fastdfsProperties.getNetworkTimeout();
      if (g_network_timeout < 0) {
        g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
      }
      g_network_timeout *= 1000; //millisecond

      g_charset = fastdfsProperties.getCharset();
      if (g_charset == null || g_charset.length() == 0) {
        g_charset = "ISO8859-1";
      }

      szTrackerServers = fastdfsProperties.getTrackerServer();
      if (szTrackerServers == null) {
        throw new MyException("tracker_server not found");
      }
      ...
      g_tracker_http_port = fastdfsHttpProperties.getHttpPort();
      g_anti_steal_token = fastdfsHttpProperties.getHttpToken();
      if (g_anti_steal_token) {
        g_secret_key = fastdfsHttpProperties.getHttpSecretKey();
      }
      g_connection_pool_enabled = fastdfsPoolProperties.getEnabled();
      g_connection_pool_max_count_per_entry = fastdfsPoolProperties.getMaxCountPerEntry();
      g_connection_pool_max_idle_time = fastdfsPoolProperties.getMaxIdleTime();
      ...
      g_connection_pool_max_wait_time_in_ms = fastdfsPoolProperties.getMaxWaitTimeInMs();
